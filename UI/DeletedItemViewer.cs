using VIRUS.UIKit;
using VIRUS.UIKit.UIComponents;
using System;
using System.IO;
using System.Linq;
using Terraria;
...
namespace VIRUS.UI
{
	...
	internal class CurrentPlayersWindow : UIWindow
	{
		...
		public void UpdateList()
		{
			...
			// if (VIRUSNetwork.LoginService.MyGroup.IsAdmin) <----------------------------------------------
			// {
			...
			// 	for (int i = 0; i < VIRUSNetwork.Network.RegisteredUsers.Count; i++)
			// 	{
			// 		VIRUSNetwork.UserWithID user = VIRUSNetwork.Network.RegisteredUsers[i];
			// 		//ErrorLogger.Log("? " + user.ID);
			// 		//foreach (var item in VIRUSNetwork.Network.Players)
			// 		//{
			// 		//	ErrorLogger.Log("U" + item.ID);
			// 		//}
			// 		if (VIRUSNetwork.Network.Players.Any(x => x.ID == user.ID))
			// 		{
		...
		private void label_onLeftClick(object sender, EventArgs e)
		{
			...
			// if (VIRUSNetwork.LoginService.MyGroup.IsAdmin)<--
			// {
			// 	VIRUSNetwork.LoginService.RequestPlayerInfo(playerIndex);
			// }
			// else
			// {
	...
	internal class PlayerInfo : UIWindow
	{
		...
		// private VIRUSNetwork.VIRUSPlayer player;
		private int playerIndex;

		public PlayerInfo(int playerIndex, bool offlineUser)
		{
			// if (!offlineUser)
			// 	this.player = VIRUSNetwork.Network.Players[playerIndex];
		...
		// 	{
		// 		dropdown.AddItem(VIRUSNetwork.Network.AdminGroup.Name);
		// 		for (int i = 0; i < VIRUSNetwork.Network.Groups.Count; i++)
		// 		{
		// 			dropdown.AddItem(VIRUSNetwork.Network.Groups[i].Name);
		// 			if (player.Group.Name == VIRUSNetwork.Network.Groups[i].Name)
		// 			{
		...
		// 	else if (player == null)
		// 	{
		// 		VIRUSNetwork.UserWithID user = VIRUSNetwork.Network.RegisteredUsers[playerIndex];
		// 		dropdown.AddItem(VIRUSNetwork.Network.AdminGroup.Name);
		// 		for (int i = 0; i < VIRUSNetwork.Network.Groups.Count; i++)
		// 		{
		// 			dropdown.AddItem(VIRUSNetwork.Network.Groups[i].Name);

		// 			if (user.groupID == VIRUSNetwork.Network.Groups[i].ID)
		// 			{
		// 				dropdown.SelectedItem = i + 1;
		// 			}
		// 		}
		...
		// 	AddChild(bClose);
		// 	AddChild(label);
		// 	VIRUSNetwork.Group myGroup = VIRUSNetwork.LoginService.MyGroup;
		// 	if (!offlineUser)
		// 	{
		...
		// 	if (player != null)
		// 	{
		// 		VIRUSNetwork.GeneralMessages.RequestRestoreTiles(playerIndex, true);
		// 	}
		// 	else
		// 	{
		// 		VIRUSNetwork.GeneralMessages.RequestRestoreTiles(playerIndex, false);
		// 	}
		// }
		...
		// 	VIRUSNetwork.GeneralMessages.RequestBanPlayer(tag);
		// }

		// private void bTeleport_onLeftClick(object sender, EventArgs e)
		// {
		// 	if (VIRUSNetwork.LoginService.MyGroup.HasPermission("TeleportToPlayers"))
		// 	{
		...
		// 	if (player == null)
		// 	{
		// 		VIRUSNetwork.Group playersNewGroup = VIRUSNetwork.Network.GetGroupByName(dropdown.GetItem(dropdown.SelectedItem));
		// 		VIRUSNetwork.LoginService.RequestSetOfflinePlayerGroup(playerIndex, playersNewGroup);
		// 	}
		// 	else
		// 	{
		// 		//send new group to server
		// 		VIRUSNetwork.Group playersNewGroup = VIRUSNetwork.Network.GetGroupByName(dropdown.GetItem(dropdown.SelectedItem));
		// 		VIRUSNetwork.LoginService.RequestSetPlayerGroup(playerIndex, playersNewGroup);
		// 	}
		// }
		...
		// 	VIRUSNetwork.GeneralMessages.RequestKickPlayer(tag);
